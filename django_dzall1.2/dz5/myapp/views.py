...
# Представление для детализированного просмотра клиента
def client_detail(request, client_id):
    client = get_object_or_404(Client, id=client_id)
    orders = Order.objects.filter(client=client)
    return render(request, 'myapp/client_detail.html', {'client': client, 'orders': orders})

# Представление для детализированного просмотра продукта
def product_detail(request, product_id):
    product = get_object_or_404(Product, id=product_id)
    orders = Order.objects.filter(products=product)
    return render(request, 'myapp/product_detail.html', {'product': product, 'orders': orders})

# Представление для детализированного просмотра заказа
def order_detail(request, order_id):
    order = get_object_or_404(Order, id=order_id)
    products = order.products.all()
    return render(request, 'myapp/order_detail.html', {'order': order, 'products': products})
